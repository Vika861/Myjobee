...
$data = '{
  "query": "query { verifyOTP(mobile_number: {country_code: \"91\", number: \"'.$num.'\"}, OTP: \"'.$otp.'\", user_type: 0) { data { is_registered user_data { _id email token mobile_number{ country_code number} whatsapp_enabled profile_type is_suspended suspension_reason last_login device_id failed_attempts timezone image OTP valid_till registration_step} } status error httpStatusCode }}",
  "variables": {}
}';
...
$iid = $json['data']['verifyOTP']['data']['user_data']['_id'];
$token = $json['data']['verifyOTP']['data']['user_data']['token'];
$httpStatus = $json['data']['verifyOTP']['httpStatusCode'];
$error = $json['data']['verifyOTP']['error'];
...
<font color='green'>OTP Verified Successfully..
	</font></div><br>
";  
...
<form action='create.php' method='POST'>
   ...
   <input type='submit' class='submit_btn' name='submit' value='Create Profile'>
...
echo "<div class='error'>  <p>looks! like something went wrong!</p> <br><p>Message :- $error</p>
  </div>
";  
